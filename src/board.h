...
private:
    ...
    std::mutex lock;
    QSoundEffect *move_sound=new QSoundEffect;
    QSoundEffect *victory_sound=new QSoundEffect;
public:
...
signals:
    void win(bool side);
    void myMove(const std::pair<int, int> from, const std::pair<int, int> to);
    void addtext(Pos p,QString nob);
public slots:
    void onSetup(Cell**);
